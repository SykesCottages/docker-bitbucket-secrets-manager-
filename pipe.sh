...
get_secrets(){
  ...
  aws secretsmanager get-secret-value --secret-id ${SECRET} --query SecretString --output text --region ${REGION}  --profile ${PROFILE} | jq -r 'to_entries|map("\(.key)=\(.value|tostring)")|.[]' > ${FILE}
}
